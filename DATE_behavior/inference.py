...
raw_datasets = load_dataset("Helsinki-NLP/opus_books", "en-it")

#print(raw_datasets)

first_row = raw_datasets['train'][0]

#print(first_row)

#print(first_row['translation'])
#print(first_row['translation']['en'])

train_datasets      = raw_datasets['train'][0:600]
validation_datasets = raw_datasets['train'][600:800]
test_datasets       = raw_datasets['train'][800:1000]

#print(type(train_datasets))
#print(train_datasets['translation'][0])
english_sentences = []
italian_sentences = []
for i in range(1):
  #print(raw_datasets['train'][i]['translation'])
  english_sentences.append(raw_datasets['train'][i]['translation']['en'])
  italian_sentences.append(raw_datasets['train'][i]['translation']['it'])
...
tokenized_datasets = tokenizer(
  ...
  padding = "max_length",
  truncation = True,
  max_length = 64, #subh 512, # Hardcoded
  return_tensors="pt"
)
...
#print(tokenized_datasets['token_type_ids'].shape)
#print(tokenized_datasets['attention_mask'].shape)

import torch
#print(torch.max(tokenized_datasets['input_ids']))
...
# input_ids = torch.randint(0, vocab_size, (1, max_length))  # Example input
word_embedding_layer = nn.Embedding(vocab_size, d_model)  # DATE
positional_embedding_layer = nn.Parameter(torch.zeros(1, max_length, d_model)) # DATE
x = word_embedding_layer(input_ids)+positional_embedding_layer[:, :input_ids.size(1), :]
dropout_layer = nn.Dropout(dropout)
x = dropout_layer(x)

print("x.shape")
print(x.shape)
#print('input embedding write: ')
#print(x)
